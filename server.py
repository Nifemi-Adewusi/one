...
def pop(arr, N, indexClient, valueClient):
    if indexClient < 0 or indexClient >= N:
        print("Index Error")
    elif valueClient > arr[indexClient] or valueClient < 0:
        print("It Is Impossible to subtract less value")
    else:
        arr[indexClient] -= valueClient
...
while True:
    ...
    if winner:
        print("Client Wins")
        connection.send("Client Wins".encode("ascii"))
        break
    ...
    if winner:
        print("Server Wins")
        connection.send("Server Wins".encode("ascii"))
        break
